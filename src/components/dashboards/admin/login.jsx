...
const AdminLogin = () => {
    ...
    async function Login() {
        try {
            ...
            const response = await axiosInstance.post('/adminApi/login', {email, password});
    ...
    async function CheckLogin() {
        try {
            const response = await axiosInstance.post('/adminApi/checkLogin', {});
            if(response.data.response === 'unauthorized'){
               setNewAccount(response.data.newAccount)
               setLoading(false) 
            }else if(response.data.response === 'connected'){
    ...
    async function ResetPassword() {
        try {
            setLoadingLogin(true)
            const response = await axiosInstance.post('/adminApi/resetPassword', {email});
            setLoadingLogin(false)
            if(response.data.response === 'done'){
                // redirect to login page
                    ...
                    <section className="w-full h-full py-14 px-6 gap-8 flex flex-col rounded-ee-xl rounded-se-xl bg-white" style={{padding: '15px 25px', display: type === 'login' ? 'flex' : 'none'}}>
                       ...
                       <Typography variant="h5" sx={{'fontFamily':'Cairo','fontWeight':'bold','direction':'rtl', marginBottom: '-25px', marginTop: '25px'}}>السلام عليكم، تسعدنا رؤيتك مجددًا</Typography>
                        ...
                        <form className="flex flex-col items-center justify-center gap-3" style={{flex: 1, alignSelf: 'center', width: '100%', maxWidth: '450px' }}>
                            ...
                            <LoadingButton loading={loadingLogin} loadingPosition="center" disabled={validator.isEmail(email) && password.length > 7 ? false : true} variant="primary" onClick={() => Login()} sx={{'paddingX':'2.5rem'}}> {newAccount === true ? 'إنشاء حساب' : 'تسجيل الدخول'} </LoadingButton>
                        </form>
                    </section>
